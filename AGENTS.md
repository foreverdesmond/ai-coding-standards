# AGENTS.md — Context L1 标准入口

> 规范版本：V3.0
> 文档状态：已审核通过（定稿 2026-08-24，Richy 终审通过）
> 作者：Tiffany-Dev（V3.0 修订；初版=WorkBuddy）
> 创建日期：2026-08-20
> 最后更新：2026-08-24
> 审核人：Richy（已审核）

---

## 1. 定位

本文件是本仓库的 **Context L1 标准入口**（物理载体），为在本仓库上工作的 AI 编码 Agent（执行载体由当前载体策略制品指定）提供一致、可预测的项目级操作说明。

它回答的问题是：进入这个仓库的 Agent 应该知道哪些跨任务稳定的约定、边界和规则。

它与 `文档规范/06-开发上下文包生成与使用规范.md` 衔接：`AGENTS.md` 是 Context L1 的载体，`06` 规定 Context L1/L2/L3 的分层、触发、失效与对账规则。

## 2. 层级与优先级（closest-wins）

`AGENTS.md` 采用分层 + 就近覆盖（closest-wins）机制：

```text
全局上下文入口                 # 跨所有仓库的全局约定（宿主路径由运行环境映射）
  → ./AGENTS.md               # 项目级：本仓库约定（本文件）
    → ./<subdir>/AGENTS.md    # 子包级：子目录专属约定（按需）
```

冲突消解优先级（从高到低）：

1. **用户当前显式指令** —— 最高，覆盖一切；
2. **就近 `AGENTS.md`** —— 离被编辑文件最近的 `AGENTS.md` 优先；
3. **远程文档 / 上级 `AGENTS.md`** —— 作为兜底补充。

Agent 应读取"离当前工作文件最近的一份 `AGENTS.md`"，而非无差别加载全部层级。

## 3. 项目概览

| 项 | 说明 |
|---|---|
| 仓库性质 | **多 Agent 开发协作规范库**（纯文档仓库，无源代码工程） |
| 技术栈 | 纯 Markdown（`.md`），无构建/编译/测试命令 |
| 规范现状 | V3.0 已审核通过（定稿 2026-08-24；V2.5 为上一已审核基线）；正文升级在 `3.0` 分支进行 |
| 调度模式 | **Hermes 常驻总调度** + 载体策略制品指定的多执行载体平行协作 |

## 4. 分支策略

| 分支 | 角色 | 说明 |
|---|---|---|
| `main` | 稳定分支 | 已审核基线，由独立合并任务在授权后合入 |
| `V2.5`（远程大写） | 当前开发分支 | Hermes 迁移开发主分支 |

**分支命名警示**：历史 V2.5 分支本地名与远程名大小写不一致（推送须显式 refspec，具体命令见实例运行记录）；V3.0 修订在 `3.0` 分支进行。

## 5. Git 与工具权限（沙箱分级）

依据《Hermes能力边界清单》§6，工具权限按沙箱分级：

| 任务类型 | sandbox | 说明 |
|---|---|---|
| 所有 Codex 派发（开发/集成/文档/Review/验证） | `danger-full-access` | V3.0 统一权限；Reviewer/Validator 受「代码不可变约束」（09 §7.3）：可构建可测试，不得改业务源码/commit 候选/merge |

规则：

- V2.5 的只读沙箱实测无法编译运行测试 → V3.0 起统一 `danger-full-access`，以代码不可变约束 + 隔离 detached 验证工作区 + 前后 HEAD/tree 对账对冲权限放大；
- **Hermes 不代执行 git**：worktree 创建/commit/merge 由相应角色自办，Hermes 只派发参数、巡检、判 gate；
- 存在冲突的任务不设并行，实施中真冲突需 Richy 协调（《Hermes流程与边界决议》C2）；
- **调度并发**：调度权由 CoordinatorEpoch 唯一持有；cron 与交互会话按 09 §12.3 移交协议换主，禁止双驱动；
- **派发门禁**：每次派发前过派发前置门禁（fail-closed），并记录 PolicyVersion + PolicyArtifactDigest。

## 6. 文档编写约定

本仓库所有规范文档遵循统一约定：

- **文档头**：规范版本、文档状态、作者、创建日期、最后更新、审核人、修订记录（文末表）；
- **文档状态**：`草稿` / `待审核` / `修改中` / `已审核通过` / `待同步` / `已取代` / `已归档`；
- **版本规则**：以 `05-文档审核与变更控制规范.md` §5 为准（`V主.次`）；
- **术语**：代码基线统一使用 `CodeBaseSHA`（旧名 `BaseSHA` 已废弃）；
- **事实与推断分离**：已验证事实、推断、建议、待确认必须分开记录。

## 7. 与本仓库规范的衔接

| 本文件作用 | 对应规范 |
|---|---|
| Context L1 载体 | `文档规范/06-开发上下文包生成与使用规范.md` |
| 上下文包生成责任与时机 | `Hermes流程与边界决议.md` §三（Doc/Design Reviewer，开发前生成、依赖已满足） |
| 调度与台账 | `文档规范/09-Hermes调度与运行时台账规范.md` |
| 角色与提示词 | `文档规范/07-子Agent任务委派与提示词规范.md` + `提示词模板/` |

---

## 修订记录

| 版本 | 日期 | 作者 | 变更说明 |
|---|---|---|---|
| V1.0 | 2026-08-20 | WorkBuddy | 首次创建：确立 Context L1 标准入口、closest-wins 优先级、项目概览、分支策略、沙箱分级、文档约定 |
| V1.1 | 2026-08-20 | WorkBuddy | §3 规范现状更新为 V2.5 待审核（V2.3 为上一已审核基线） |
| V2.5 定稿 | 2026-08-20 | WorkBuddy | 评审通过，标记为 V2.5 正式基线 |
| V2.5 勘误 | 2026-08-21 | WorkBuddy | §3 规范现状由「V2.5 待审核」修正为「V2.5 已审核通过」，与文档头及定稿记录一致 |

| V3.0-draft | 2026-08-24 | Hermes | V3.0 修订同步（提案 v5）：§3 规范现状改 V3.0-draft；§5 沙箱分级改为统一 danger-full-access + Reviewer 代码不可变约束；新增调度并发（CoordinatorEpoch 移交协议）与派发前置门禁说明。详见 `3.0` 分支 V3.0-proposal.md |

| V3.0 定稿 | 2026-08-24 | Tiffany-Dev | Richy 宣布「V3.0 整体审核通过」：版本头升 V3.0/已审核通过；十轮审核（提案 v1-v5 + 正文九轮收口）全部关闭；D0/D1 残留清零验收达成；证据包 E1-E8 + Canary 11/11 PASS 存档 |