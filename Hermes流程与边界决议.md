...
> 状态：**已归档（历史决议依据，非规范性入口）**
> 性质：与 V3.0-proposal.md 同等定位——记录历史讨论与决策依据，不作为现行可执行规则；
> 文中飞书知识库、TG、deepseek 等均为历史部署事实，现行规则以 `文档规范/` 正文为准。
> 日期：2026-08-20
> 参与：Richy + Hermes（Tiffany），延续历史评审讨论与迁移设计（相关调研/审阅/升级计划文档已归档至外部知识库，位置登记于实例记录）
...
| A2 | 事件源与巡检 | **推送事件源 + 定时对账兜底双通道**；推送可能丢(已实证)，需定时对账兜底。频率/实现登记于实例记录 |
...
> 修订：2026-08-20，Hermes——清理对已归档历史文档的悬空书名号引用。
...
## V3.0 附录（修订：Tiffany-Dev，2026-08-24）：调度并发控制模型（2026-08-24）

V2.5 假定「单机单实例、无协调租约锁」；V3.0 起调度权可能由协调 cron 与交互会话竞争持有，
并发模型变更为 **CoordinatorEpoch（FencingToken）**：

- 任一时刻仅一个 Epoch 所有者；旧 Epoch 的派发/消费一律拒绝；
- 换主 = 原子条件更新（Epoch+StateRevision 匹配才生效）；
- 正常移交双确认；旧实例失联超 LostOwnerTimeout 后经 Richy 授权恢复接管；
- 完整协议见 09 §12.3；载体策略变更通道见 09 §12.4 与 05 §6.3。