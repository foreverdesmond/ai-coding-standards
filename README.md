...
- [x] DOC-DEC-013：任务完成采用开发任务文档状态驱动；周期对账消费文档信号并按需定向取证。
- [x] DOC-DEC-014：新增 `09` 作为控制协议；项目共享本地 JSON 作为任务运行状态唯一实时真源，开发任务文档保存 Git 管理的持久快照。
- [x] DOC-DEC-015：线程状态、任务状态和证据状态分离；UI 标签和 Git HEAD 不得替代事件结论。
- [x] DOC-DEC-016：高风险调度必须使用共享本地 JSON、短时锁、版本复查、原子更新工具、状态指纹、幂等派发键和协调租约。
- [x] DOC-DEC-017：明确要求 Codex 子任务时，创建失败不得以子 Agent 或当前任务自行执行兜底。
- [x] DOC-DEC-018：Review 的执行状态与代码 Verdict 分离，`NotRun` 不自动等于 Review 阻塞。
