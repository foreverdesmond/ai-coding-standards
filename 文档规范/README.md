...
| 文件 | 核心职责 |
|---|---|
| [01-任务背景与现状分析文档规范](./01-任务背景与现状分析文档规范.md) | 建立当前事实、运行拓扑、证据和风险基线 |
| [02-需求规格文档规范](./02-需求规格文档规范.md) | 定义范围、行为、验收层级和成功标准 |
| [03-详细设计文档规范](./03-详细设计文档规范.md) | 设计组件、运行时所有权、失败语义和测试策略 |
| [04-详细开发任务文档规范](./04-详细开发任务文档规范.md) | 拆分任务、依赖、文件所有权、交界责任和状态 |
| [05-文档审核与变更控制规范](./05-文档审核与变更控制规范.md) | 管理批准、版本、变更传播、候选冻结和证据有效性 |
| [06-开发上下文包生成与使用规范](./06-开发上下文包生成与使用规范.md) | 规定上下文包的生成、校验、探索、失效和对账 |
| [07-子Agent任务委派与提示词规范](./07-子Agent任务委派与提示词规范.md) | 规定角色契约、提示词结构、权限和输出状态 |
| [08-开发验证、代码审查与合并门禁规范](./08-开发验证、代码审查与合并门禁规范.md) | 规定 Level 0–3、Review 分工、分支门禁和合并资格 |

角色提示词模板位于 [`提示词模板`](./提示词模板/README.md)。模板是委派起点，不得替代任务实际基线和自主探索。
...
- [ ] DOC-DEC-001：采用 Level 0–3 验证等级及对应状态语义。
- [ ] DOC-DEC-002：Level 0–3 是通用状态门禁；迭代、集成和稳定分支映射作为推荐默认策略，由项目配置覆盖。
- [ ] DOC-DEC-003：Level 2 默认允许项目负责人在项目指定的集成候选上人工运行，发现问题后按返工闭环处理。
- [ ] DOC-DEC-004：Level 3 和最终合并审核通过后，按项目策略进入稳定候选；默认申请稳定分支合并。
- [ ] DOC-DEC-005：默认禁止 Agent 自动创建、删除、清空或重建数据库及其他持久资源；例外需精确目标的单次授权。
- [ ] DOC-DEC-006：上下文包和子 Agent 角色提示词分别形成独立规范。
- [ ] DOC-DEC-007：规范只定义角色能力和分支逻辑角色，不绑定具体模型、供应商或项目分支名称。
- [ ] DOC-DEC-008：采用轻量、标准、高风险三条路线；Context L2 仅按风险触发。
- [ ] DOC-DEC-009：Context L1 只在稳定事实实质变化时更新；`NoChange` 不建立独立任务或 Review。
- [ ] DOC-DEC-010：SOLID 保持所有路线的硬门禁，证据展开程度可以随风险调整。
- [ ] DOC-DEC-011：真实编码任务默认使用独立任务分支和不可变提交作为 Review 候选；普通开发者只可向自己的任务分支提交。
- [ ] DOC-DEC-012：迭代集成与主分支合并分别由独立执行任务完成，Reviewer 只审核资格，不执行合并。
- [ ] DOC-DEC-013：任务完成采用事件驱动处理，周期巡检只作为健康检查兜底。
