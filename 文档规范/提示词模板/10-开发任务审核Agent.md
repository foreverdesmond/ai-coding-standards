...
- 检查派发前强制清单、文档状态生产/消费、周期巡检、中断门禁和 Review 循环升级是否可执行；
- 检查唯一 CanonicalTaskDocumentPath、SharedRuntimeStatePath、StateUpdateToolPath、状态 Schema/版本/锁、DispatchKey、协调租约、暂停、RecoveryOnly、冷恢复和 Canary 是否可执行；
- 检查明确要求 Codex 子任务时是否禁止子 Agent 或当前线程兜底；
